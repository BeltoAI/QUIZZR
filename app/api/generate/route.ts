...
const DEBUG = process.env.QUIZZR_DEBUG === '1';
...
function logErr(...args: any[]) { if (DEBUG) console.error(...args); }
...
function isTriviallyEmpty(s: string): boolean {
  const t = (s || '').trim();
  if (!t) return true;
  // Some local models reply with this exact sentence
  if (/^no extra content\.?$/i.test(t)) return true;
  // Or they echo markers only
  if (/^###BEGIN_JSON###\s*###END_JSON###$/i.test(t)) return true;
  return false;
}
...
// Plain deterministic template fallback
...
function parseQuizFromJsonish(content: string): Quiz {
  if (isTriviallyEmpty(content)) throw new Error('empty');
  ...
  log('[QUIZZR] CONTENT LEN:', content.length);
  log('[QUIZZR] EXTRACTED LEN:', repaired.length);
...
function parseQuizFromPlainTemplate(topic: string, difficulty: 'easy'|'medium'|'hard', n: number, plain: string): Quiz {
  if (isTriviallyEmpty(plain)) throw new Error('empty');
...
export async function POST(req: NextRequest) {
  ...
  if (!isTriviallyEmpty(text)) {
    try {
      ...
      logErr('[QUIZZR] json attempt failed:', (e as Error).message);
    }
  }

  // 2) “repair” attempt (only if prior content existed)
  if (!isTriviallyEmpty(text)) {
...
Return only between markers.`;
    const t2 = await callLLM_text(p2, llmUrl);
    if (!isTriviallyEmpty(t2)) {
      try {
        const quiz = parseQuizFromJsonish(t2);
        return new Response(JSON.stringify({ mode: 'json-fixed', quiz }), { status: 200, headers: { 'Content-Type': 'application/json' } });
      } catch (e) {
        logErr('[QUIZZR] json-fixed attempt failed:', (e as Error).message);
      }
    }
  }

  // 3) Final fallback: plain deterministic template
  ...
  try {
    ...
    return new Response(JSON.stringify({
      ...
      hint: 'Enable QUIZZR_DEBUG=1 to see logs. Try fewer questions, shorter source, or a different local model.',
      preview: (plain || text || '').slice(0, 800),
    }), { status: 422 });
  }
}
